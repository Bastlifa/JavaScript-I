...
console.log(`Car 33 is a ${inventory.filter(invEntry => invEntry.id === 33)
    .map(car => car.car_year)} ${inventory.filter(invEntry => invEntry.id === 33)
    .map(car => car.car_make)} ${inventory.filter(invEntry => invEntry.id === 33)
...
BMWAndAudi.forEach((car) => console.log(JSON.stringify(car)));
